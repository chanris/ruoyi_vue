...
public class SysLoginService
{
     ...
     * @return 结果
     */
    public String loginEmail(String email, String code, String uuid) {

        validateEmailVerifyCode(email, code, uuid); // 取缓存中的验证码进行对比
        ...
        try {
            // 该方法会去调用UserDetailsServiceImpl.loadUserByUsername
            authentication = authenticationManager
                    .authenticate(new EmailCodeAuthenticationToken(email));
        } catch (Exception e) {
            e.printStackTrace();
            if (e instanceof BadCredentialsException) {
     ...
     * @param uuid
     */
    public  void validateEmailVerifyCode(String email, String code, String uuid) {
        String verifyKey = CacheConstants.CAPTCHA_EMAIL_CODE_KEY + StringUtils.nvl(uuid, "");
        String captcha = redisCache.getCacheObject(verifyKey);
        redisCache.deleteObject(verifyKey);
        if (captcha == null)
        {
